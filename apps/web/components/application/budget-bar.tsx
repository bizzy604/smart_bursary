		...
		<section className="rounded-2xl border border-secondary/30 bg-background p-5 shadow-xs">
			<div className="flex flex-wrap items-end justify-between gap-2">
				<div>
					...
					<h2 className="mt-1 font-serif text-lg font-semibold text-primary">{programName}</h2>
				</div>
				<p className="text-sm text-muted-foreground">Ceiling: {formatCurrencyKes(ceiling)}</p>
			</div>

			<div className="mt-4 h-4 overflow-hidden rounded-full bg-muted">
				<div className="relative h-full w-full">
					<div className="absolute inset-y-0 left-0 bg-secondary/70" style={{ width: `${allocatedPercent}%` }} />
					<div className="absolute inset-y-0 left-0 bg-county-primary" style={{ width: `${disbursedPercent}%` }} />
				</div>
			</div>

			<div className="mt-3 grid gap-2 text-sm text-foreground/90 sm:grid-cols-3">
