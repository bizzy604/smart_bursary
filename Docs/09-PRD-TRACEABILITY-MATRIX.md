...
|---|---|---|---|---|---|---|
| TM-01 | Multi-tenant county isolation | Implemented | apps/api/common/decorators/county.decorator.ts, apps/api/modules/application/application-audit.service.ts, apps/api/test/integration/b08-security-audit.e2e-spec.ts | County-scoped access is now regression-tested for cross-county and cross-ward denial on audit and reporting surfaces. | B-08 | BE |
| TM-02 | Configurable county brand profile | Implemented | apps/api/modules/tenant/tenant.controller.ts, apps/api/modules/tenant/tenant.service.ts, apps/web/app/(admin)/settings/branding/page.tsx, apps/web/components/layout/county-branding-provider.tsx, apps/web/lib/application-pdf.tsx, apps/api/test/integration/tenant-settings.e2e-spec.ts | County-admin branding API + UI now persist and propagate county identity tokens to county-facing surfaces and generated PDF payloads. | B-03 | BE+FE |
| TM-03 | Platform operator county provisioning | Implemented | apps/api/modules/tenant/tenant-provisioning.controller.ts, apps/api/modules/tenant/provisioning.service.ts, apps/api/modules/tenant/dto/provision-county.dto.ts, apps/api/test/integration/tenant-provisioning-plan-gates.e2e-spec.ts | Platform operators can now provision counties, seed wards, and bootstrap county-admin access through dedicated APIs. | B-04 | BE |
| TM-04 | Plan-tier feature gating | Implemented | apps/api/common/decorators/plan-tier.decorator.ts, apps/api/common/guards/plan-tier.guard.ts, apps/api/modules/tenant/tenant.controller.ts, apps/api/modules/ai/ai.controller.ts, apps/api/test/integration/tenant-provisioning-plan-gates.e2e-spec.ts | API-level feature gates now enforce tier eligibility for restricted and enterprise-only capabilities. | B-04 | BE |
...
| AU-04 | RBAC roles enforced | Implemented | apps/api/common/guards/roles.guard.ts, apps/api/modules/application/application.controller.ts, apps/api/modules/document/document.controller.ts, apps/api/test/integration/b08-security-audit.e2e-spec.ts | Student-only endpoints now declare explicit role metadata and are covered by integration assertions. | B-08 | BE |
| AU-05 | Ward admin scope restriction | Implemented | apps/api/modules/review/ward-review.service.ts, apps/api/modules/application/application-audit.service.ts, apps/api/modules/reporting/reporting.controller.ts, apps/api/test/integration/b08-security-audit.e2e-spec.ts | Ward admins are restricted to ward-scoped data, while county-wide reporting routes now reject ward-admin access. | B-08 | BE |
...
| AF-06 | County form customization settings | Implemented | apps/web/app/(admin)/settings/page.tsx, apps/api/modules/tenant/dto/update-settings.dto.ts, apps/api/modules/tenant/tenant.service.ts, apps/api/test/integration/tenant-settings.e2e-spec.ts, apps/web/app/(admin)/settings/page.test.tsx | County admins can now configure approved colour scheme, logo placement, and section ordering via persisted tenant settings. | B-03 | BE+FE |
...
| AI-05 | County-configurable scoring weights | Implemented | apps/api/modules/ai/ai.controller.ts, apps/api/modules/ai/scoring-weights.service.ts, apps/api/queue/processors/ai-scoring.processor.ts, apps/web/app/(admin)/settings/ai-scoring/page.tsx, apps/api/test/integration/tenant-settings.e2e-spec.ts, apps/web/app/(admin)/settings/ai-scoring/page.test.tsx | County admins can now read/update weights in UI, and queue scoring payloads apply county-configured weights for subsequent scoring runs. | B-03 | BE+FE |
| AI-06 | AI never auto-approves/rejects | Implemented | apps/api/modules/ai/ai-score.service.ts, apps/api/modules/internal/internal.controller.ts, apps/api/test/integration/b08-security-audit.e2e-spec.ts | AI ingest transitions are asserted to remain non-terminal in integration coverage. | B-08 | BE |
| RW-01 | Workflow statuses with timeline audit | Implemented | apps/api/modules/application/application-audit.service.ts, apps/api/modules/application/application.controller.ts, apps/api/test/integration/b08-security-audit.e2e-spec.ts | Dedicated timeline retrieval endpoint now exposes ordered workflow history with role-aware scope checks. | B-08 | BE |
...
| RW-04 | SMS on every status change | Implemented | apps/api/modules/notification/notification.module.ts, apps/api/modules/notification/notification-lifecycle.service.ts, apps/api/modules/notification/sms.service.ts, apps/api/queue/queue.service.ts, apps/api/queue/processors/sms.processor.ts, apps/api/test/integration/notification-status-change.e2e-spec.ts | Submission, review, and disbursement transitions now enqueue SMS notifications with persisted delivery records and county audit visibility via `GET /notifications/deliveries`. | B-07 | BE |
| RW-05 | Review notes in audit trail | Implemented | apps/api/modules/application/application-audit.service.ts, apps/api/modules/application/application.controller.ts, apps/api/test/integration/b08-security-audit.e2e-spec.ts | Review notes are now queryable via API with cross-ward and cross-county access controls. | B-08 | BE |
| DB-01 | Trigger M-Pesa B2C disbursement | Implemented | apps/api/modules/disbursement/disbursement.controller.ts, apps/api/modules/disbursement/disbursement.service.ts, apps/api/modules/disbursement/disbursement-queue.service.ts, apps/api/modules/disbursement/disbursement-execution.service.ts, apps/api/modules/disbursement/mpesa.service.ts, apps/api/test/integration/disbursement-execution.e2e-spec.ts | Queue-backed M-Pesa execution now transitions payouts through success/failure lifecycle with persisted transaction IDs and status visibility. | B-05 | BE |
...
12. Phase 5A: Disbursement execution/retry/receipt completion (completed)
